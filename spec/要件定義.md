...
- 機能: ethers.jsでブロックチェーンからAgentCard検索 + .well-known/agent.jsonからスキーマ取得
- 入力: category, maxPrice, minReputation
- 内部処理:
  1. オンチェーンからAgentCard取得（Base URL、価格情報含む）
  2. 各エージェントの `${url}/.well-known/agent.json` を取得
  3. agent.jsonからAPIスキーマ（skills、OpenAPI等）を取得
- 出力: 拡張AgentCard配列（スキーマ情報含む）
  例: [{ name: "FlightAgent", price: 0.01, url: "...", skills: [...], rating: 4.5 }]

// execute_agent_capability Tool
- 機能: エージェント機能の実行（A2A + x402 + 再送を内部で完結）
- 入力: agentBaseUrl, message, userId, maxPrice
- 内部処理:
  1. .well-known/agent.jsonからA2Aエンドポイントを取得
  2. A2A JSON-RPC呼び出し（fetch）
  3. HTTP 402受信時、請求額とmaxPriceを比較
  4. 範囲内の場合: Privyウォレット取得
  5. EIP-3009署名作成
  6. X-PAYMENTヘッダー構築
  7. 決済付きA2A再送（自動）
  8. 範囲外の場合: エラー返却
...
- オンチェーンにAgentCard登録（Base URL、価格情報含む）
...
sequenceDiagram
    ...
    MCP-->>C: 拡張AgentCard一覧（スキーマ含む）

    C->>C: 価格確認・予算チェック
    C->>C: 最適エージェント選択（価格考慮）

    loop 各エージェント
        C->>MCP: execute_agent_capability(agentBaseUrl, message, userId, maxPrice)
...
if (flightAgent.price <= userBudget && flightAgent.price <= dailyLimit) {
  ...
  await execute_agent_capability({
    agentBaseUrl: flightAgent.url, // Base URL
    message: 'パリ行きの航空券',
...
5. Claudeエージェントが自動実行:
   ...
   - [MCP: discover_agents] オンチェーンからBase URL取得 + agent.json取得
...
[MCP: discover_agents] オンチェーンから3件のAgentCard取得
[MCP: discover_agents] .well-known/agent.jsonから詳細スキーマ取得中...
[MCP: discover_agents] 3件のエージェントを発見（スキーマ情報含む）
...
[MCP: execute_agent_capability] agent.jsonからエンドポイント取得
...
```typescript
{
  ...
  description: "ブロックチェーンからAgentCardを検索し、.well-known/agent.jsonからスキーマ情報を取得",
  inputSchema: {
    type: "object",
    properties: {
  ...
  handler: async ({ category, maxPrice, minReputation }) => {
    ...
    const filtered = agents.filter(
      a => a.payment.pricePerCall <= maxPrice && a.averageRating >= minReputation
    );

    // 2. 各エージェントのagent.jsonを取得（並列処理）
    const enrichedAgents = await Promise.all(
      filtered.map(async (agent) => {
        try {
          // .well-known/agent.jsonを取得
          const agentJsonUrl = `${agent.url}/.well-known/agent.json`;
          const agentJson = await fetch(agentJsonUrl).then(r => r.json());
          ...
          return {
            ...
            url: agent.url,
            endpoint: agentJson.endpoint,  // A2Aエンドポイント
            skills: agentJson.skills,      // inputSchemaを含む詳細スキル情報
            openapi: agentJson.openapi,    // OpenAPIリンク（オプション）
            price: ethers.formatUnits(agent.payment.pricePerCall, 6),
            rating: agent.totalRatings / agent.ratingCount,
            category: agent.category
          };
        } catch (error) {
          // agent.json取得失敗時はオンチェーン情報のみ返却
          console.warn(`Failed to fetch agent.json for ${agent.name}:`, error);
          return {
            agentId: agent.agentId,
            name: agent.name,
            description: agent.description,
            url: agent.url,
            endpoint: null,
            skills: agent.skills,  // オンチェーンの基本情報のみ
            price: ethers.formatUnits(agent.payment.pricePerCall, 6),
            rating: agent.totalRatings / agent.ratingCount,
            category: agent.category
          };
...
```typescript
{
  ...
  inputSchema: {
    type: "object",
    properties: {
      agentBaseUrl: { type: "string", description: "エージェントのBase URL" },
    ...
    required: ["agentBaseUrl", "message", "userId", "maxPrice"]
  },
  handler: async ({ agentBaseUrl, message, userId, maxPrice }) => {
    // 0. agent.jsonからA2Aエンドポイントを取得
    const agentJsonUrl = `${agentBaseUrl}/.well-known/agent.json`;
    const agentJson = await fetch(agentJsonUrl).then(r => r.json());
    const a2aEndpoint = agentJson.endpoint || `${agentBaseUrl}/api/v1/agent`;

    // 1. 初回A2Aリクエスト
    const initialResponse = await fetch(a2aEndpoint, {
      method: "POST",
    ...
    if (initialResponse.status === 402) {
      ...
      // 3. 決済付きで自動再送
      const retryResponse = await fetch(a2aEndpoint, {
        method: "POST",
        headers: {
