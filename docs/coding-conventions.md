...
- **サービス間相関**: web → Agent Service は `x-request-id` ヘッダーで requestId を、リクエストボディの `conversationId` で会話 ID を伝播する（`agent-service-client` / orchestrator が自動付与）。受信側では形式検証を通った値だけログコンテキストに採用する（信用しない入力）。conversationId はトレースメタデータであり、Agent Service のロジックには使わない。
