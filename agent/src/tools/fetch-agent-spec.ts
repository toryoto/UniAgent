...
const fetchAgentSpecSchema = z.object({
  agentUrl: z
    .string()
    .describe(
      'エージェントのベースURL（discover_agents の結果から取得）。例: https://example.com/api/agents/flight'
    ),
});
...
export const fetchAgentSpecTool = tool(
  async (input: z.infer<typeof fetchAgentSpecSchema>) => {
    try {
      logger.logic.info('Fetching agent spec', { agentUrl: input.agentUrl });

      // 1. agent.json を取得
      const agentJson = await fetchAgentJson(input.agentUrl);
      if (!agentJson) {
        return JSON.stringify({
          success: false,
          error: `agent.json を取得できませんでした: ${input.agentUrl}`,
        });
      }
