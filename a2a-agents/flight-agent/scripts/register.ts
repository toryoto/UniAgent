/**
 * Flight Agent — Pinata IPFS upload + AgentIdentityRegistry on-chain registration.
 * Usage:
...
const AGENT_SLUG = 'flight-agent';
const AGENT_NAME = 'FlightSearchAgent';
...
const AGENT_IMAGE =
  'https://via.placeholder.com/150/1a73e8/ffffff?text=FlightSearch';
...
async function main(): Promise<void> {
  const BASE_URL = normalizeBaseUrl(process.env.BASE_URL ?? 'http://localhost:3005');
  const RPC_URL = process.env.RPC_URL ?? '';
  const PRIVATE_KEY = process.env.PRIVATE_KEY;
  const WALLET_ADDRESS = process.env.AGENT_RECEIVER_ADDRESS ?? '';

  console.log('=== Flight Agent Registration ===');
  ...
  const ipfsUri = await uploadToIPFS(metadata, `flight-agent-${AGENT_SLUG}`);
