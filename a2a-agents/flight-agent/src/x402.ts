...
const RESOURCE_PATH = '/flight-agent';
...
class ExpressHttpAdapter {
  ...
  getUrl(): string {
    const host = this.req.get('host') ?? `localhost:${process.env.PORT ?? '3005'}`;
...
export async function createFlightX402HttpServer(): Promise<x402HTTPResourceServer> {
  ...
  const routeConfig: RouteConfig = {
    ...
    description: 'Flight Search Agent execution (x402)',
